...
*   **NumPy-Optimized Rendering:** For CPU-heavy environments or pre-training, generation is driven by highly optimized NumPy and SciPy operations (e.g., `fftconvolve`, `map_coordinates`), using **Numba (@njit)** for fast sub-pixel grid painting.
...
*   **Data:** Vectorized synthetic images generated via a NumPy/Numba renderer. Star fluxes and counts are drawn from a realistic Dynamic Bulge Luminosity Function.
