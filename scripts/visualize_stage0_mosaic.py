...
def visualize_mosaic_optimized(mosaic_idx=0, data_dir="data/bulge_stage0_full/mosaics"):
    ...
    psf_path = os.path.join(data_dir, f"mosaic_{mosaic_idx:03d}_psf.npy")
    ...
    if os.path.exists(psf_path):
        psf_1x = np.load(psf_path)
        sum_p = np.sum(psf_1x)
        eff_area = (sum_p**2) / (np.sum(psf_1x**2) + 1e-9)
        print(f"📊 Using PSF from file: eff_area={eff_area:.2f}")
    else:
        eff_area = 12.0
        print(f"⚠️ PSF file not found, using default eff_area.")
    ...
    if 'snr' in structured_cat.dtype.names:
        ...
        actual_pixel_values = star_signal[np.clip(y0, 0, star_signal.shape[0]-1), np.clip(x0, 0, star_signal.shape[1]-1)]
        local_background = np.maximum(0, actual_pixel_values - fluxes * 0.2) 
        noise_var = fluxes + eff_area * (local_background + 25.0)
    ...
    ax0.set_title(f"Full Mosaic ({star_signal.shape[1]}x{star_signal.shape[0]})\n{len(targets_x):,} Targets (SNR >= 5)")
    ...
    zoom_size = 256
    zx, zy = star_signal.shape[1]//2 - 128, star_signal.shape[0]//2 - 128
    ax1 = plt.subplot2grid((2, 4), (1, 2))
    crop = network_input[zy:zy+zoom_size, zx:zx+zoom_size]
    im1 = ax1.imshow(crop, cmap='magma', origin='lower')
    ...
    plt.tight_layout()
    output_png = "mosaic_visualisation.png"
    plt.savefig(output_png, dpi=150)
    plt.close()
    print(f"✅ Mosaic visualisation saved to {output_png}")
